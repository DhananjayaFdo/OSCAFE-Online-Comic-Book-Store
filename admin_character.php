...
<?php 
    if(isset($_POST['remove_item'])){
        $delete = $_POST['id'];
        $remove = "DELETE FROM heros WHERE id = '$delete';";
...
<body>
    ...
    <div class="form_container">
        <form action="" method="post" class="form"  enctype="multipart/form-data">
            <h1>ADD HEROS</h1>
            <input type="text" name="name" placeholder="Enter Hero Name" class="input" required>
            <input type="text" name="description" placeholder="Enter Heros short Descri.." class="input" >
        ...
        <?php  
            if(isset($_POST['submit']) && $_FILES['images']){
                ...
                $comucDescri = $_POST['description'];
                $comicFranchise = $_POST['franchise_select'];

                //Comic Image File Data
                $imageName = $_FILES['images']['name'];
                $tempName = $_FILES['images']['tmp_name'];
                $fileLocation = "images/Characters/". $imageName;

                //uplead user upload image top server
                move_uploaded_file($tempName,$fileLocation);

                $sql = "INSERT INTO heros (name, descripion, franchise, image) VALUES('$comicName','$comucDescri','$comicFranchise','$imageName');";
